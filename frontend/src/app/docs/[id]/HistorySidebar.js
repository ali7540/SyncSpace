...
export default function HistorySidebar({ documentId, isOpen, onClose, onPreview, isReadOnly, Content }) {
  ...
  useEffect(() => {
    if (isOpen) {
      setLoading(true);
      api.get(`/docs/${documentId}/versions`)
        .then(res => {
    ...
    <div className="fixed inset-y-0 right-0 w-80 bg-white shadow-2xl border-l border-gray-200 z-50 flex flex-col transform transition-transform">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
            ...
            versions.map((v) => (
                <div key={v.id} className="border border-gray-200 rounded-lg p-3 hover:border-blue-300 hover:shadow-sm transition-all group">
                    ...
                    <p className="text-xs text-gray-500 mb-3">
                        Saved by <span className="text-gray-700">{v.author?.name || 'Unknown'}</span>
                    </p>
